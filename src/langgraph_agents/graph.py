...
class LegalExtractionGraph:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        # 添加边（线性流程）
        workflow.add_edge("ner", "coref")
        workflow.add_edge("coref", "normalization")
        workflow.add_edge("normalization", "relation")
        workflow.add_edge("relation", "qa")
    ...
    ) -> Literal["end", "backtrack_ner", "backtrack_coref", "backtrack_normalization", "backtrack_relation"]:
        """
        决定是否回溯以及回溯到哪个阶段
    ...
    def extract(self, text: str, pdf_path: str = None) -> Dict[str, Any]:
        ...
        Returns:
            抽取结果
        """
        logger.info(f"开始抽取流程，文本长度: {len(text)} 字符")
        ...
        try:
            ...
            logger.info(f"  识别关系: {len(final_state.get('relations', []))}")
